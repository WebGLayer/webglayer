...
function visualize(data){	
	...
	heatmap2.renderer.colors.set([  1, 1, 1, 1.2, 
	 ...
	 heatmap2.gradFunction = function() {
		...
		/*Vehicle type*/
		var vt   = {data: data.veh_type,  domain: data.vehtypeEnum ,  name: 'vt', type:'ordinal', label : "vehicle type"};	
		var vtd = new WGL.ChartDiv("right","vt", "Vehicle type");
		vtd.setDim(WGL.addOrdinalHistDimension(vt));
		WGL.addLinearFilter(vt,20, 'vtF');
		charts['vt']   = new  WGL.ui.StackedBarChart(vt, "vt", "Vehicle type",'vtF');
	
		/*Ageband*/
		var ab   = {data: data.age_band,  domain: data.abEnum ,  name: 'ab', type:'ordinal', label : "age band"};	
		var abd = new WGL.ChartDiv("right","ab", "Age Band");
		abd.setDim(WGL.addOrdinalHistDimension(ab));
		WGL.addLinearFilter(ab, 13, 'abF');
		charts['ab']   = new  WGL.ui.StackedBarChart(ab, "ab", "Age Band",'abF');
